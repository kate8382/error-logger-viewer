...
describe('Table - CRUD operations', () => {
  beforeEach(() => {
    ...
    cy.intercept('DELETE', '**/errors/*', { statusCode: 200 }).as(
      'deleteError',
    );
  ...
  it('редактирует первую ошибку через модальное окно', () => {
    ...
    cy.window().then((win) => {
      if (
        win.app &&
        win.app.errorApi &&
        typeof win.app.errorApi.setMode === 'function'
      ) {
        win.app.errorApi.setMode('server');
      }
      if (
        win.errorTableInstance &&
        typeof win.errorTableInstance.setMode === 'function'
      ) {
    ...
    cy.get('#modal textarea.modal__comment-area')
      .should('exist')
      .clear()
    ...
    cy.window().then((win) => {
      console.log(
        'DEBUG app mode:',
        win.app && win.app.errorApi && win.app.errorApi.mode,
      );

      console.log(
        'DEBUG errorApi baseUrl:',
        win.app && win.app.errorApi && win.app.errorApi.baseUrl,
      );
    });
    ...
    cy.window().then((win) => {
      if (win.localStorage.getItem('app_mode') === 'demo') {
        const errs = JSON.parse(
          win.localStorage.getItem('errorsLocal') || '[]',
        );
