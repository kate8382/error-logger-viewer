...
const db = new Low(adapter, { errors: [], projects: [], users: [] });
...
await db.write();
console.log(`[server] DB loaded: ${db.data.errors.length} errors, ` +
...
function requireApiKey(req, res, next) {
    ...
    const headerKey = sanitizeString(req.headers['x-api-key'], 128);
    const bodyKey = sanitizeString(req.body?.apiKey, 128);
...
app.post('/projects', writeLimiter, async (req, res) => {
    const body = req.body || {};
    ...
    if (Array.isArray(body.members)) {
        members = body.members.map((m) => sanitizeString(m, 200)).filter(Boolean);
    }
...
app.get('/projects', async (req, res) => {
    ...
    if (req.query.owner) {
        const ownerQ = sanitizeString(req.query.owner, 200);
        projects = projects.filter((p) => p.owner === ownerQ || (Array.isArray(p.members) && p.members.includes(ownerQ)));
    }
...
app.post('/users', writeLimiter, async (req, res) => {
    const body = req.body || {};
...
app.get('/errors/stats', async (req, res) => {
    ...
    const by = sanitizeString(req.query.by, 20);
    const group = sanitizeString(req.query.group, 20);
    ...
    if (safeBy === 'status') {
        ...
        errors.forEach((e) => {
            ...
            if (!periodKey)
                return; // skip records with no/invalid date
            if (!periodResult[periodKey])
                periodResult[periodKey] = {};
...
app.get('/errors', async (req, res) => {
    ...
    const filterKeys = Object.keys(req.query).filter((k) => !RESERVED_KEYS.has(k) && ALLOWED_FILTER_KEYS.has(k));
        ...
        errors = errors.filter((e) => {
            ...
            return filterKeys.every((key) => {
                ...
                return (val !== undefined &&
                    String(val).toLowerCase().includes(sanitizeString(req.query[key], 200).toLowerCase()));
            });
        });
    }
    // Legacy ?filter= parameter (filters by type).
    if (req.query.filter) {
        const filterVal = sanitizeString(req.query.filter, 100).toLowerCase();
        errors = errors.filter((e) => String(e.type || '').toLowerCase() === filterVal);
    }
    // Sorting.
    if (req.query.sort) {
        const order = sanitizeString(req.query.order, 10) === 'desc' ? -1 : 1;
        const sortKey = sanitizeString(req.query.sort, 50);
        if (sortKey === 'status') {
 ...
 */
app.post('/errors', errorIngestLimiter, async (req, res) => {
    ...
    db.data.projects = db.data.projects || [];
    // --- Project resolution ---
    const headerApiKey = sanitizeString(req.headers['x-api-key'], 128);
    const bodyApiKey = sanitizeString(body.apiKey, 128);
    const bodyProjectId = sanitizeString(body.projectId, 128);
    let project = null;
    if (headerApiKey)
        project = findProjectByApiKey(headerApiKey);
    if (!project && bodyApiKey)
        project = findProjectByApiKey(bodyApiKey);
    if (!project && bodyProjectId)
        project = findProjectById(bodyProjectId);
    if (!project) {
        ...
        const bodyAny = body;
        const maybeEmail = sanitizeString(bodyAny.owner || body.user, 200);
    ...
    const found = db.data.errors.find((e) => String(e.projectId || 'unknown') === String(projectId) &&
 ...
 */
app.put('/errors/:id', requireApiKey, writeLimiter, async (req, res) => {
    ...
    const rawStatus = sanitizeString(body.status, 20);
 ...
 */
app.delete('/errors/:id', requireApiKey, writeLimiter, async (req, res) => {
    await db.read();
    if (!db.data?.errors) {
        return res.status(404).json({ error: 'No errors found.' });
    }
