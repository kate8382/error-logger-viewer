...
The production image uses a multi-stage build: the builder stage installs devDependencies and compiles the frontend/backend, while the runtime stage installs only production dependencies and runs the compiled backend.
...
Production Compose stores the LowDB file in a named Docker volume through `DB_FILE=/app/data/db.json`, adds `restart: unless-stopped`, and includes a healthcheck for `/errors`.
