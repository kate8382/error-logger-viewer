...
(Cypress.Commands as any).add('clickSettingsOption', (group: string, value: string, opts: { force?: boolean } = {}) => {
  ...
  return cy.get(sel).scrollIntoView().click({ force }) as unknown as Cypress.Chainable<any>;
});
...
(Cypress.Commands as any).add('openRowActions', (index: number = 0) => {
  ...
  return cy
    ...
    .then(() => {
      ...
      return cy.get('#errorTableBody').find('tr').eq(index).should('have.descendants', 'button') as unknown as Cypress.Chainable<any>;
    });
});
